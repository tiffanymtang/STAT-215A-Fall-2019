...
For this in-class lab, we will look into clustering with some winery data from the UC machine learning repository ([link](https://archive.ics.uci.edu/ml/datasets/wine)). It contains information on the chemical composition of 178 wines
from 3 cultivars. Using this data, we will explore and evaluate different clusterings of this data. 
...
Without having known the true cultivar groupings, we may wonder if it is possible to recover these cultivar groupings via clustering (i.e., in an unsupervised fashion). We will first try clustering the raw data using k-means and plot the clusters, projected onto the first two PCs.
